...
pub async fn execute(
    ...
    let summary = runner
        .run(&collection, None, RunMode::Sequential, env_vars.as_ref(), None)
        .await?;
    ...
    match format {
        OutputFormat::Human => {
            ...
            for r in &summary.results {
                let icon = if r.error.is_some() {
                    "❌"
                } else if r.test_result.as_ref().map(|t| t.status) == Some(reqforge_core::testing::TestStatus::Passed) {
                    "✅"
                } else {
                    "❌"
                };
        ...
        OutputFormat::Json => {
            let json = reqforge_core::testing::reporter::generate_json(&[/* would need TestResult list */]).unwrap_or_default();
            println!("{}", json);
        }
        OutputFormat::Junit => {
            let xml = reqforge_core::testing::reporter::generate_junit_xml(&[/* would need TestResult list */]).unwrap_or_default();
            println!("{}", xml);
        }
    }
